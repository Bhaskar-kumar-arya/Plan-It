...
  // ... (store and state setup remain the same) ...
...
    if (type === 'bin') {
      // ... (bin logic is unchanged)
      position = { x: 0, y: 0 };
      displayType = 'bin';
      status = 'idea';
    } else {
      // --- ✅ MODIFIED 'add' / 'connect' LOGIC ---
      displayType = 'canvas';
      status = 'confirmed';
...
        // This is our new shortcut! Use the position from the payload.
        position = modalPayload.position;
      } else {
        // 'add' type from toolbar: Place in the center of the view
...
    // ... (socket emit and auto-select logic is unchanged) ...
...
  S         placeholder="e.g., 'Eiffel Tower'"
          />
          <button
            type="submit"
Source code           className="w-full mt-4 px-4 py-2 bg-accent text-white font-semibold rounded-md hover:bg-accent-hover transition-colors"
          >

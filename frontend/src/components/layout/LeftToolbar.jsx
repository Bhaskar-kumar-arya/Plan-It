...
const LeftToolbar = () => {
  ...
  const openAddLocationModal = useTripStore((state) => state.openAddLocationModal);
    ...
    <aside className="w-16 bg-background-secondary border-r border-border p-3 z-20">
      <div className="flex flex-col items-center gap-3">
        ...
        <ToolButton
          icon={MapPin}
          label="Add Location"
          // ✅ This tool is no longer 'active', it's an action
          isActive={false} 
          onClick={() => openAddLocationModal({ type: 'add' })} // ✅ Open modal
        />
        <ToolButton
          icon={StickyNote}
          label="Add Note"
